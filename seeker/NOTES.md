...
- **`router.refresh()` re-renders server components but does NOT reset client `useState`.** The IJP editor kept pre-accept values after confirming a suggestion; keying the component by IJP version forces the remount. Watch for this pattern on any editor fed by server props.
